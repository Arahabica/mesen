  ...
  const download = useCallback(() => {
    ...
    img.onload = () => {
      ...
      mozaicDrawing.areas.forEach(area => {
        ...
        const mosaicWidth = Math.min(img.width, img.height) * 0.03
        ...
        for (let x = gridMinX; x < gridMaxX; x += mosaicWidth) {
          for (let y = gridMinY; y < gridMaxY; y += mosaicHeight) {
            ...
            const sampleX = Math.min(x + mosaicWidth / 2, img.width - 1)
            const sampleY = Math.min(y + mosaicHeight / 2, img.height - 1)
            const sampleWidth = Math.min(1, img.width - sampleX)
            const sampleHeight = Math.min(1, img.height - sampleY)
